...
// ---------- Types ----------
type TemplateCategory = 'layout' | 'industry';
type FilterCategory = 'all' | TemplateCategory;
type AIHelperMode = 'bullets' | 'keywords' | 'metrics' | 'skills';

interface Template {
  id: 'modern' | 'traditional' | 'creative' | 'minimal' | 'tech-focused';
  name: string;
  description: string;
...
const TEMPLATES: Template[] = [
  {
    ...
    colors: { primary: '#3B82F6', secondary: '#1E40AF', accent: '#8B5CF6', background: '#FFFFFF', text: '#1F2937' },
  ...
  },
  {
    ...
    colors: { primary: '#374151', secondary: '#4B5563', accent: '#6B7280', background: '#FFFFFF', text: '#111827' },
  ...
  },
  {
    ...
    colors: { primary: '#000000', secondary: '#333333', accent: '#666666', background: '#FFFFFF', text: '#000000' },
...
const AI_HELPER_OPTIONS: ReadonlyArray<{ id: AIHelperMode; label: string; icon: LucideIcon }> = [
...
const EnhancedResumeGenerator: React.FC = () => {
  const [portfolioData, setPortfolioData] = useState<PortfolioData>({
    ...
    contactForm: {
      ...
      fields: { name: true, email: true, company: false, message: true },
    },
  });

  const [selectedTemplate, setSelectedTemplate] = useState<Template['id']>('modern');
  const [showTemplateSelector, setShowTemplateSelector] = useState(false);
  const [showDownloadMenu, setShowDownloadMenu] = useState(false);
  const [filterCategory, setFilterCategory] = useState<FilterCategory>('all');
  const [atsOnly, setAtsOnly] = useState(false);
  const [showLivePreview, setShowLivePreview] = useState(true);
  const [activeSection, setActiveSection] = useState<'personal' | 'experience' | 'skills' | 'projects' | 'education' | 'contact'>('personal');
  const [showAIHelper, setShowAIHelper] = useState(false);
  const [aiHelperType, setAIHelperType] = useState<AIHelperMode>('bullets');
  ...
  // UI classes
  const inputClasses =
    'w-full px-3 py-2 border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent placeholder-gray-400 dark:placeholder-gray-300';
  const textareaClasses =
    'w-full px-3 py-2 border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none placeholder-gray-400 dark:placeholder-gray-300';
  const selectClasses =
    'px-3 py-2 border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white rounded-lg text-sm focus:ring-2 focus:ring-blue-500';
  ...
  const generateAIContent = () => {
    ...
    setTimeout(() => {
      ...
      switch (aiHelperType) {
        case 'bullets': {
          const topic = aiInput || 'solution';
          const bulletExamples = [
            `• Developed and implemented ${topic} resulting in improved efficiency and user satisfaction`,
            `• Led cross-functional team to successfully deliver ${topic} ahead of schedule and under budget`,
            `• Optimized ${topic} which resulted in 30% reduction in operational costs`,
            `• Collaborated with stakeholders to define requirements and deliver high-quality ${topic}`,
            `• Mentored junior team members and established best practices for ${topic}`,
          ];
  ...
  const suggestSkillsForRole = (role: string) => {
    ...
    let technical: string[] = ['Git', 'CI/CD', 'Agile', 'REST APIs'];
    let soft: string[] = ['Communication', 'Problem Solving', 'Teamwork', 'Leadership', 'Time Management'];
    let tools: string[] = ['Jira', 'Slack', 'VS Code'];
  ...
  // ---------- Form handlers (typed) ----------
  const updatePersonalInfo = <K extends keyof PortfolioData>(field: K, value: PortfolioData[K]) => {
  ...
  const updateExperience = (index: number, field: keyof Experience, value: Experience[typeof field]) => {
    setPortfolioData((prev) => ({
      ...prev,
  ...
  const addSkillGroup = () => {
    setPortfolioData((prev) => ({ ...prev, skills: [...prev.skills, { category: '', skills: [] }] }));
  };

  const updateSkillGroup = (index: number, field: keyof SkillGroup, value: SkillGroup[typeof field]) => {
    setPortfolioData((prev) => ({
      ...prev,
      skills: prev.skills.map((sg, i) => (i === index ? { ...sg, [field]: value } : sg)),
    }));
  };
  ...
  const addSkillToGroup = (groupIndex: number) => {
    ...
    setPortfolioData((prev) => ({
      ...prev,
      skills: prev.skills.map((sg, i) => (i === groupIndex ? { ...sg, skills: [...sg.skills, newSkill] } : sg)),
    }));
  };

  const updateSkillInGroup = (groupIndex: number, skillIndex: number, field: keyof Skill, value: Skill[typeof field]) => {
    setPortfolioData((prev) => ({
      ...prev,
      skills: prev.skills.map((sg, i) =>
        i === groupIndex ? { ...sg, skills: sg.skills.map((s, j) => (j === skillIndex ? { ...s, [field]: value } : s)) } : sg,
      ),
    }));
  };
  ...
  const removeSkillFromGroup = (groupIndex: number, skillIndex: number) => {
    setPortfolioData((prev) => ({
      ...prev,
      skills: prev.skills.map((sg, i) => (i === groupIndex ? { ...sg, skills: sg.skills.filter((_, j) => j !== skillIndex) } : sg)),
    }));
  };

  const removeSkillGroup = (index: number) => {
    setPortfolioData((prev) => ({ ...prev, skills: prev.skills.filter((_, i) => i !== index) }));
  };

  const addProject = () => {
    setPortfolioData((prev) => ({
      ...prev,
      projects: [...prev.projects, { name: '', description: '', technologies: [], url: '', github: '' }],
    }));
  };

  const updateProject = (index: number, field: keyof Project, value: Project[typeof field]) => {
    setPortfolioData((prev) => ({
      ...prev,
  ...
  const addEducation = () => {
    setPortfolioData((prev) => ({ ...prev, education: [...prev.education, { degree: '', institution: '', year: '', gpa: '' }] }));
  };
  ...
  const removeEducation = (index: number) => {
    setPortfolioData((prev) => ({ ...prev, education: prev.education.filter((_, i) => i !== index) }));
  };

  const updateContactForm = <K extends keyof ContactForm>(field: K, value: ContactForm[K]) => {
    setPortfolioData((prev) => ({ ...prev, contactForm: { ...prev.contactForm, [field]: value } }));
  };

  const updateContactFormField = (fieldName: keyof ContactFormFields, value: boolean) => {
    setPortfolioData((prev) => ({
      ...prev,
      contactForm: { ...prev.contactForm, fields: { ...prev.contactForm.fields, [fieldName]: value } },
    }));
  };
...
<style>
  ...
  .gradient-bg { background: linear-gradient(135deg, ${currentTemplate.colors.primary}, ${currentTemplate.colors.accent}); }
</style>
</head>
...
<script>
  ...
  const generateMarkdownPortfolio = () => {
    ...
    if (portfolioData.skills.length > 0) {
      markdown += `## Skills\n\n`;
      portfolioData.skills.forEach((sg) => {
        markdown += `### ${sg.category}\n\n`;
        sg.skills.forEach((s) => {
          const stars = '⭐'.repeat(s.level);
          markdown += `- **${s.name}**: ${stars}\n`;
        });
        markdown += `\n`;
      });
    }
    ...
    if (portfolioData.experience.length > 0) {
      ...
      portfolioData.experience.forEach((exp) => {
        ...
        exp.description.forEach((d) => (markdown += `- ${d}\n`));
        markdown += `\n`;
      });
    }
    ...
    if (portfolioData.projects.length > 0) {
      ...
      portfolioData.projects.forEach((p) => {
        markdown += `### ${p.name}\n\n${p.description}\n\n`;
        if (p.technologies.length > 0) markdown += `**Technologies:** ${p.technologies.join(', ')}\n\n`;
        if (p.url) markdown += `🔗 [Live Demo](${p.url})\n`;
        if (p.github) markdown += `📁 [Source Code](${p.github})\n`;
        markdown += `\n`;
      });
    }
  ...
  const downloadAsHTML = () => {
    ...
    const file = new Blob([generateHTMLPortfolio()], { type: 'text/html' });
  ...
  const downloadAsMarkdown = () => {
    const element = document.createElement('a');
    const file = new Blob([generateMarkdownPortfolio()], { type: 'text/markdown' });
    element.href = URL.createObjectURL(file);
  ...
  const SkillRatingStars = ({ level, onLevelChange }: { level: number; onLevelChange: (lvl: number) => void }) => (
      ...
      {[1, 2, 3, 4, 5].map((starLevel) => (
        <button
          ...
          className={`w-5 h-5 rounded transition-colors ${
            level >= starLevel ? 'text-yellow-400 hover:text-yellow-500' : 'text-gray-300 hover:text-gray-400'
          }`}
        >
      ...
      <form className="space-y-4">
        ...
        {portfolioData.contactForm.fields.message && (
          <div>
            ...
            <textarea rows={4} className={textareaClasses} placeholder="Your message..." />
          </div>
        )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredTemplates.map((template) => (
              <button
                key={template.id}
                type="button"
                className={`relative border rounded-xl p-6 text-left transition-all hover:shadow-lg ${
              ...
              >
                <div className="h-24 rounded-lg mb-4 flex items-center justify-center text-white font-semibold" style={{ backgroundColor: template.colors.primary }}>
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    {template.atsOptimized && (
                      <span className="px-2 py-1 bg-green-100 text-green-800 dark:bg-green-800 dark:text-green-200 text-xs rounded-full">ATS</span>
                    )}
                  </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl max-w-3xl w-full max-h-[85vh] overflow-hidden flex flex-col">
        <div className="p-6 border-b border-gray-200 dark:border-gray-700">
          ...
          <div className="flex flex-wrap gap-2">
            {AI_HELPER_OPTIONS.map(({ id, label, icon: Icon }) => (
              <button
                key={id}
                type="button"
                onClick={() => {
        ...
        <div className="p-6 space-y-4 overflow-y-auto flex-1">
          <div>
            ...
            <textarea
              ...
              placeholder={
                aiHelperType === 'bullets'
                  ? 'E.g., Built a web application for tracking inventory'
                  : aiHelperType === 'keywords'
                  ? 'Paste the full job description here...'
                  : aiHelperType === 'metrics'
                  ? 'E.g., Led project to improve customer satisfaction'
          ...
          {aiOutput && (
            <div className="mt-4">
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-2">💡 Tip: Review and customize this content to match your experience</p>
            </div>
          )}
        </div>
  ...
  const LivePreview = () => (
    <div className="w-full h-full bg-white dark:bg-gray-800 rounded-lg shadow-lg overflow-hidden">
      <div className="h-full overflow-y-auto" style={{ backgroundColor: currentTemplate.colors.background, color: currentTemplate.colors.text }}>
        {!portfolioData.name && !portfolioData.role ? (
          ...
          <div className="p-6">
            <div
              ...
              style={{ background: `linear-gradient(135deg, ${currentTemplate.colors.primary}, ${currentTemplate.colors.accent})`, color: 'white' }}
            >
                              ...
                              <div className="flex items-center space-x-2">
                                <div className="w-32 h-2 bg-gray-200 dark:bg-gray-600 rounded-full">
                                  <div
                                    ...
                                    style={{ width: `${(skill.level / 5) * 100}%`, backgroundColor: currentTemplate.colors.primary }}
                                  />
                                </div>
                                <div className="flex space-x-1">
                                  {[1, 2, 3, 4, 5].map((level) => (
                                    <Star key={level} className={`w-3 h-3 ${skill.level >= level ? 'text-yellow-400' : 'text-gray-300'}`} fill={skill.level >= level ? 'currentColor' : 'none'} />
                                  ))}
                                </div>
                              </div>
                            </div>
      ...
      <div className="flex h-[calc(100vh-80px)]">
        <div className={`${showLivePreview ? 'w-1/2' : 'w-full'} bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 overflow-y-auto transition-all duration-300`}>
          <div className="p-6">
              ...
              {[
                ...
                <button
                  ...
                  className={`flex items-center space-x-2 px-4 py-2 rounded-md text-sm font-medium transition-colors whitespace-nowrap ${
                    activeSection === id ? 'bg-white dark:bg-gray-600 text-blue-600 dark:text-blue-400 shadow-sm' : 'text-gray-600 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white'
                  }`}
                >
                  <Icon className="w-4 h-4" />
                  <span>{label}</span>
            ...
            {/* --- sections omitted for brevity; they are identical to your original, using typed handlers above --- */}
            {/* PERSONAL */}
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 gap-4">
                    <div>
                      ...
                      <input type="text" value={portfolioData.name} onChange={(e) => updatePersonalInfo('name', e.target.value)} className={inputClasses} placeholder="Enter your full name" />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Job Title/Role</label>
                      <input type="text" value={portfolioData.role} onChange={(e) => updatePersonalInfo('role', e.target.value)} className={inputClasses} placeholder="e.g., Full Stack Developer" />
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Email</label>
                        <input type="email" value={portfolioData.email} onChange={(e) => updatePersonalInfo('email', e.target.value)} className={inputClasses} placeholder="your.email@example.com" />
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Phone</label>
                        <input type="tel" value={portfolioData.phone} onChange={(e) => updatePersonalInfo('phone', e.target.value)} className={inputClasses} placeholder="(555) 123-4567" />
                      </div>
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Location</label>
                      <input type="text" value={portfolioData.location} onChange={(e) => updatePersonalInfo('location', e.target.value)} className={inputClasses} placeholder="City, State/Country" />
                    </div>
                    <div className="grid grid-cols-1 gap-4">
                      <div>
                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Website (Optional)</label>
                        <input type="url" value={portfolioData.website || ''} onChange={(e) => updatePersonalInfo('website', e.target.value)} className={inputClasses} placeholder="https://yourwebsite.com" />
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">LinkedIn (Optional)</label>
                        <input type="url" value={portfolioData.linkedin || ''} onChange={(e) => updatePersonalInfo('linkedin', e.target.value)} className={inputClasses} placeholder="https://linkedin.com/in/yourprofile" />
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">GitHub (Optional)</label>
                        <input type="url" value={portfolioData.github || ''} onChange={(e) => updatePersonalInfo('github', e.target.value)} className={inputClasses} placeholder="https://github.com/yourusername" />
                      </div>
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Professional Summary</label>
                      <textarea value={portfolioData.summary} onChange={(e) => updatePersonalInfo('summary', e.target.value)} rows={4} className={textareaClasses} placeholder="Write a brief summary about yourself, your experience, and your goals..." />
                    </div>
                  </div>
                </div>
              </div>
            ...
            {activeSection === 'experience' && (
              <div className="space-y-6">
                <div className="flex items-center justify-between">
                  ...
                  <button type="button" onClick={addExperience} className="flex items-center space-x-2 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">
                    ...
                    {portfolioData.experience.map((exp, index) => (
                      <div key={index} className="border border-gray-200 dark:border-gray-700 dark:bg-gray-750 rounded-lg p-6">
                        <div className="flex items-center justify-between mb-4">
                          ...
                          <button type="button" onClick={() => removeExperience(index)} className="p-2 text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors">
                        ...
                        <div className="grid grid-cols-1 gap-4">
                          <div className="grid grid-cols-2 gap-4">
                            ...
                            <div>
                              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Company</label>
                              <input type="text" value={exp.company} onChange={(e) => updateExperience(index, 'company', e.target.value)} className={inputClasses} placeholder="Company name" />
                            </div>
                          </div>
                          ...
                          <div className="grid grid-cols-2 gap-4">
                            <div>
                              ...
                              <input type="text" value={exp.startDate} onChange={(e) => updateExperience(index, 'startDate', e.target.value)} className={inputClasses} placeholder="Jan 2023" />
                            </div>
                            <div>
                              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">End Date</label>
                              <input type="text" value={exp.endDate} onChange={(e) => updateExperience(index, 'endDate', e.target.value)} className={inputClasses} placeholder="Present" />
                            </div>
                          </div>
            ...
            {activeSection === 'skills' && (
              <div className="space-y-6">
                <div className="flex items-center justify-between">
                  ...
                  <button type="button" onClick={addSkillGroup} className="flex items-center space-x-2 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">
                    <Plus className="w-4 h-4" />
                    ...
                    {portfolioData.skills.map((skillGroup, index) => (
                      <div key={index} className="border border-gray-200 dark:border-gray-700 dark:bg-gray-750 rounded-lg p-6">
                        <div className="flex items-center justify-between mb-4">
                          ...
                          <button type="button" onClick={() => removeSkillGroup(index)} className="p-2 text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors">
                            <Trash2 className="w-4 h-4" />
                          </button>
                        </div>
                        ...
                        <div className="space-y-4">
                          ...
                          <div>
                            <div className="flex items-center justify-between mb-3">
                              ...
                              <button type="button" onClick={() => addSkillToGroup(index)} className="text-sm text-blue-600 hover:text-blue-700 flex items-center space-x-1">
                            ...
                            {skillGroup.skills.length === 0 ? (
                              <p className="text-sm text-gray-500 dark:text-gray-400 py-4 text-center">No skills added yet. Click "Add Skill" to start.</p>
                            ) : (
                              <div className="space-y-3">
                                {skillGroup.skills.map((skill, skillIndex) => (
                                  <div key={skillIndex} className="flex items-center space-x-3 bg-gray-50 dark:bg-gray-700 p-3 rounded-lg">
                                    ...
                                    <div className="flex items-center space-x-2">
                                      ...
                                      <SkillRatingStars level={skill.level} onLevelChange={(level) => updateSkillInGroup(index, skillIndex, 'level', level)} />
                                    </div>
            ...
            {activeSection === 'projects' && (
              <div className="space-y-6">
                <div className="flex items-center justify-between">
                  ...
                  <button type="button" onClick={addProject} className="flex items-center space-x-2 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">
                    <Plus className="w-4 h-4" />
                    ...
                    {portfolioData.projects.map((project, index) => (
                      <div key={index} className="border border-gray-200 dark:border-gray-700 dark:bg-gray-750 rounded-lg p-6">
                        <div className="flex items-center justify-between mb-4">
                          ...
                          <button type="button" onClick={() => removeProject(index)} className="p-2 text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors">
                            <Trash2 className="w-4 h-4" />
                          </button>
                        </div>
                        ...
                        <div className="space-y-4">
                          <div>
                            ...
                            <input type="text" value={project.name} onChange={(e) => updateProject(index, 'name', e.target.value)} className={inputClasses} placeholder="My Awesome Project" />
                          </div>

                          <div>
                            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Description</label>
                            <textarea value={project.description} onChange={(e) => updateProject(index, 'description', e.target.value)} rows={3} className={textareaClasses} placeholder="Brief description..." />
                          </div>
                          ...
                          <div className="grid grid-cols-2 gap-4">
                            <div>
                              ...
                              <input type="url" value={project.url || ''} onChange={(e) => updateProject(index, 'url', e.target.value)} className={inputClasses} placeholder="https://myproject.com" />
                            </div>
                            <div>
                              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">GitHub URL (Optional)</label>
                              <input type="url" value={project.github || ''} onChange={(e) => updateProject(index, 'github', e.target.value)} className={inputClasses} placeholder="https://github.com/user/repo" />
                            </div>
                          </div>
                        </div>
                      </div>
            ...
            {activeSection === 'education' && (
              <div className="space-y-6">
                <div className="flex items-center justify-between">
                  ...
                  <button type="button" onClick={addEducation} className="flex items-center space-x-2 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">
                    <Plus className="w-4 h-4" />
                    ...
                    {portfolioData.education.map((edu, index) => (
                      <div key={index} className="border border-gray-200 dark:border-gray-700 dark:bg-gray-750 rounded-lg p-6">
                        <div className="flex items-center justify-between mb-4">
                          ...
                          <button type="button" onClick={() => removeEducation(index)} className="p-2 text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors">
                            <Trash2 className="w-4 h-4" />
                          </button>
                        </div>
                        ...
                        <div className="space-y-4">
                          <div>
                            ...
                            <input type="text" value={edu.degree} onChange={(e) => updateEducation(index, 'degree', e.target.value)} className={inputClasses} placeholder="Bachelor of Science in Computer Science" />
                          </div>
                          <div>
                            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Institution</label>
                            <input type="text" value={edu.institution} onChange={(e) => updateEducation(index, 'institution', e.target.value)} className={inputClasses} placeholder="University Name" />
                          </div>
                          <div className="grid grid-cols-2 gap-4">
                            <div>
                              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Year</label>
                              <input type="text" value={edu.year} onChange={(e) => updateEducation(index, 'year', e.target.value)} className={inputClasses} placeholder="2024" />
                            </div>
                            <div>
                              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">GPA (Optional)</label>
                              <input type="text" value={edu.gpa || ''} onChange={(e) => updateEducation(index, 'gpa', e.target.value)} className={inputClasses} placeholder="3.8" />
                            </div>
                          </div>
                        </div>
                      </div>
                    ...
                    {portfolioData.contactForm.enabled && (
                      <div className="space-y-4 ml-6">
                        ...
                        <div>
                          ...
                          <div className="space-y-2">
                            {(Object.entries(portfolioData.contactForm.fields) as [keyof ContactFormFields, boolean][]).map(([fieldName, enabled]) => (
                              <div key={fieldName} className="flex items-center space-x-3">
                                <input
                                  type="checkbox"
                                  id={`field-${String(fieldName)}`}
                                  checked={enabled}
                                  onChange={(e) => updateContactFormField(fieldName, e.target.checked)}
                                  className="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                                />
                                <label htmlFor={`field-${String(fieldName)}`} className="text-sm text-gray-700 dark:text-gray-300 capitalize">
                                  {String(fieldName)}
                                </label>
                              </div>
                            ))}
